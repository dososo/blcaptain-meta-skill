...
これは Codex 専用でも、Claude Code 専用でもありません。

BLCaptain Meta Skill の中心は標準的な Skill フォルダです：`SKILL.md` + `references/` + `assets/` + `examples/` + `evals/` + `scripts/`。ローカル Skill フォルダを読める Agent、または Agent Skills 形式の能力をサポートする Agent であれば、各プラットフォームの方法に合わせて利用できます。

| プラットフォーム / ツール | 対応方式 | 説明 |
| --- | --- | --- |
| Codex / OpenAI Agent Skills | 直接インストール | `blcaptain-meta-skill/` をローカル skills ディレクトリにコピーし、`$blcaptain-meta-skill` で呼び出します |
| Claude Skills | 互換利用 | 対象プラットフォームが要求する場所に `blcaptain-meta-skill/` をインポートまたは配置します |
| Claude Code | 互換利用 | Claude Code が本リポジトリまたは Skill フォルダを読めるようにし、`SKILL.md` とリソースディレクトリを使います |
| Skill 対応の他 Agent | 汎用方法論パッケージ | `SKILL.md` とリソースを読めるなら実行可能です。metadata は平台に合わせて調整が必要な場合があります |
| 通常のチャットボット | 直接インストール非推奨 | フォルダ、スクリプト、リソースを読めない場合は、方法論参考としてのみ使えます |

公式ドキュメントでも Agent Skills は instructions、metadata、scripts、templates、resources を含む能力パッケージとして説明されています。本プロジェクトもその考え方に沿っており、単一クライアント専用の prompt ではありません。
...
GitHub の `Code -> Download ZIP` からダウンロードして解凍しても構いません。

### 2. Codex / ローカル Agent

リポジトリ内の Skill パッケージフォルダ `blcaptain-meta-skill/` を skills ディレクトリにコピーします。
...
### 3. Claude Skills / Claude Code / その他の Agent

クライアントごとにインストール画面は異なりますが、基本手順は同じです。

1. 本リポジトリの `blcaptain-meta-skill/` フォルダをインポート、アップロード、または参照させます。
2. Agent が `blcaptain-meta-skill/SKILL.md` を読めることを確認します。
3. `references/`、`assets/templates/`、`examples/`、`evals/`、`scripts/` にアクセスできることを確認します。
4. 対象プラットフォームの metadata、インストールパス、権限を確認します。
5. 新しいセッションで呼び出します。
...
プラットフォームに Skill インポート機能がない場合は、本リポジトリをプロジェクト資料として渡し、実行前に `blcaptain-meta-skill/SKILL.md` を読むよう依頼してください。
...
これらが通れば、パッケージ構造、ルーティング例、コンテキスト予算は利用可能です。
